...
import { FurnitureModel } from "./furniture.entity";
...
class ProductApi {
    ...
    async getModels(): Promise<FurnitureModel[]> {
        const models = await getDocs(
            collection(this.db, this.modelCollectionName)
        );
        return models.docs.map((doc) => {
            const data = doc.data();
            const model: FurnitureModel = {
                ...
                PublishedAt:
                    (data.PublishedAt as Timestamp | null)?.toDate() || null,
                UserID: data.UserID,
            };
    ...
    async createMaterialProduct(
        ...
        try {
            ...
            > = {
                ...
                MaterialCategoryID: data.materialCategory,
                Brand: null,
                MaterialDescription: data.materialDescription,
                PreviewImage: null,
                UserID: userId,
