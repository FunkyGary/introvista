...
const ProjectTitleInput: React.FC<InputProps> = ({
  ...
  return (
    <div className="flex justify-center items-center overflow-auto w-full h-16 text-4xl antialiased truncate">
      <TextareaAutosize
        ref={inputRef}
        onBlur={onBlur}
        onChange={(e) => handleChange(e.target.value)}
        onKeyDown={handleKeyDown}
        className="p-3 text-center break-words resize-none focus:border-0 focus:ring-0 focus:ring-opacity-0 focus:outline-none"
        value={value}
      />
    </div>
  )
